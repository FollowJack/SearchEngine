...
//		 _stringService = new StringParserService();
	...
	public ArrayList<Document> searchVectorAngle(String searchPhrase) {
		...
		Document source = _searchService.parseToDocument(searchPhrase);
		
		// get indexer
		IndexerService indexer = new IndexerService();
		searchedDocuments = indexer.getVectorAngleMatchingResult(documents, 10);

		// return best 10 entries
		return searchedDocuments;
	}

	// public ArrayList<Document> searchBooleanInformationRetrieval(String
	// searchPhrase) {
	// ArrayList<Document> searchedDocuments = new ArrayList<Document>();
	//
	// // case: is empty
